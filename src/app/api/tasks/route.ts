...
function getEmployeeIdForUser(userId: string): number | null {
  const row = db()
...
export async function GET(req: Request) {
  ...
  if (onlyToday) whereParts.push("date(t.deadline) = date('now')");

  // оператор видит только свои задачи
  if (auth.user.role === "operator") {
    const employeeId = getEmployeeIdForUser(auth.user.id);
  ...
  const rows = db()
    .prepare(
      ...
      ${where}
      ORDER BY t.is_completed ASC, t.deadline ASC, t.created_at DESC
    `,
    )
    .all(params);
...
export async function PUT(req: Request) {
  ...
  if (auth.user.role === "operator") {
    ...
    auditLog({
      ...
      details: `Оператор отметил выполнение задачи #${parsed.data.id}: ${parsed.data.is_completed}`,
    });
...
export async function DELETE(req: Request) {
  ...
  auditLog({ actorUserId: Number(auth.user.id), action: "delete", entity: "tasks", entityId: id, details: `Удалена задача #${id}` });
  return NextResponse.json({ ok: true });
}
