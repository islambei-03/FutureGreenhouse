...
const seedUsers: SeedUser[] = [
  ...
  { full_name: "Айгуль Жумабаева", login: "aigul", password: "aigul123", role: "agronomist" },
  { full_name: "Руслан Карабаев", login: "ruslan", password: "ruslan123", role: "worker" },
...
/** Удаляет автозаполненные «Демо-сбор» и прочие тестовые культуры. */
export async function cleanupDemoCultures(pool: Pool) {
  await pool.query(`
    DELETE FROM cultures
    WHERE notes IN ('demo', 'demo-rich')
       OR name LIKE 'Демо-%'
       OR section LIKE 'Demo-%'
  `);
}
...
async function seedDomainIfEmpty(pool: Pool, a: DbAdapter) {
  ...
  `);
    ...
    // Два оператора теплиц (рабочие): ввод показаний при сбое датчиков, полив, задачи.
    const employees = TEAM_WORKERS.map((w) => ({
      full_name: w.full_name,
      position: "Рабочий" as const,
      greenhouse_id: greenhouseIds[w.greenhouseIndex]!,
      phone: w.phone,
    ...
    for (let i = 0; i < TEAM_WORKERS.length; i++) {
      ...
      const ghId = greenhouseIds[TEAM_WORKERS[i]!.greenhouseIndex];
      if (empId && ghId) await db.prepare("UPDATE greenhouses SET responsible_employee_id = ? WHERE id = ?").run(empId, ghId);
    }
