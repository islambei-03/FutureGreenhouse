...
- **`DATABASE_URL` на Vercel** должен указывать на **ту же** базу, что и локально (скопируйте строку из `.env.local`). Разница только в том, что локально часто стоит `sslmode=no-verify`; в **production** приложение **автоматически** заменяет это на `sslmode=require` при подключении — руками править строку не обязательно.
- Для локальной отладки допустимо `sslmode=no-verify`; можно сразу использовать `sslmode=require`.
