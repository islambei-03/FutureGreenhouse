          ...
          {NAV_ITEMS.filter((i) => allowed(i.allowed, role)).map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={[
                  "flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 border transition",
                  isActive
              ...
              >
                ...
                {badge ? (
                  <span className="min-w-6 h-6 px-2 rounded-full text-xs grid place-items-center bg-[color:var(--accent)] text-black">
                    {badge}
                  </span>
                ) : null}
              </Link>
